<form wire:submit="saveShippingOption" class="border border-gray-400/20 rounded-xl">
            ...
            @if ($currentStep == $step)
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    @foreach ($this->shippingOptions as $option)
                        <div>
                            <input class="hidden peer" type="radio" wire:model.live="chosenShipping"
                                name="shippingOption" value="{{ $option->getIdentifier() }}"
                                id="{{ $option->getIdentifier() }}" />

                            <label
                                class="flex items-center justify-between p-4 text-sm font-medium border border-gray-400/20 rounded-lg shadow-sm cursor-pointer peer-checked:border-scarlet-500 hover:bg-scarlet-500/20 peer-checked:ring-1 peer-checked:ring-scarlet-500"
